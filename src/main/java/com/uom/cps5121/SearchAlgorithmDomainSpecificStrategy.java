 ...
 */
public class SearchAlgorithmDomainSpecificStrategy extends SearchAlgorithmAbstract {
     ...
     * @post $none
     */
    public SearchAlgorithmDomainSpecificStrategy() {
        ...
        if (isBugged & requests < REQUEST_LIMIT) {
            ...
            for (y = southPixel; y < northPixel; y++) {
                for (x = westPixel; x < eastPixel; x++) {
                    pair = new Pair<>(x, y);
                    /**
                     * makes sure the pixel is not already checked before testing it.
                     */
